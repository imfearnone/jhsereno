...
export default function SellingProcess() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentStep((prev) => (prev < sells.length - 1 ? prev + 1 : 0)); 
    }, 2000);
  ...
  return (
    <div className="w-auto p-10">
        ...
        >
            ...
            <div className="absolute hidden md:flex inset-0 items-center justify-between mb-12 lg:mb-12 xl:mb-3">
                <div className="w-full h-1 bg-gray-200"></div>
                <div
                    className="absolute h-1 bg-hoverColor duration-300"
                    style={{
                    width: `${(currentStep / (sells.length - 1)) * 100}%`,
                    }}
                ></div>
            </div>
        </motion.div>
